...
/**
 * The written half of a challenge: the human title, who it affects, and the
 * four lenses. Separate from the problem file because it is generated by a
 * prompted step and cached, not computed on every run — and because keeping
 * the written content in its own file makes it obvious which is which.
 */
export interface Challenge {
  ...
  humanTitle: string;
  whoItAffects: string;
  lenses: Record<'build' | 'start' | 'study' | 'write', {
    proposition: string;
    specifics: string[];
    limitation: string;
  }>;
...
export const getChallenges = memo((): Map<string, Challenge> => {
  const dir = join(DATA, 'challenges');
  const out = new Map<string, Challenge>();
  if (!existsSync(dir)) return out;
  for (const file of readdirSync(dir)) {
    if (!file.endsWith('.json')) continue;
    const c = JSON.parse(readFileSync(join(dir, file), 'utf8')) as Challenge;
    out.set(c.slug, c);
  }
  return out;
});
 ...
 * number itself is still on /how-it-works. A reader gets "widely reported"
 * because "73 out of 100" answers a question nobody asked.
 */
export function surfaceLabel(problem: Problem): string {
  if (problem.score >= problem.buildThreshold) return 'Widely reported';
  if (problem.score >= 55) return 'Reported repeatedly';
  return 'Early signal';
}

/**
 * Reach as one human sentence, from the same counts that feed the score.
 *
 * "17 people raised this across 6 weeks, on both iOS and Android." carries
 * what the seven-component breakdown carried, and can be read rather than
 * decoded. Every figure in it is counted, not written.
 */
export function reachLine(problem: Problem): string {
  const people = problem.inputs.uniqueReviewers;
  const weeks = problem.inputs.weeksWithEvidence ?? 0;
  const platforms = new Set(problem.evidence.map((e) => e.platform));

  const who = `${people} ${people === 1 ? 'person' : 'people'} raised this`;
  const when = weeks > 1 ? ` across ${weeks} weeks` : '';
  const where =
    platforms.size > 1
